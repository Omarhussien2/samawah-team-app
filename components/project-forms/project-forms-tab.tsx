...
import { cn, formatRelativeAr, getProjectType, getProjectTypeBadgeClass, getProjectTypeLabel } from "@/lib/utils";
...
const SYSTEM_TEMPLATE_CATEGORY = "system";
...
function isVisibleProjectFormTemplate(template: Pick<ProjectFormTemplate, "category" | "applies_to_path">) {
  return template.category !== SYSTEM_TEMPLATE_CATEGORY && !isTrainingTemplate(template);
}
...
export function ProjectFormsTab({ project, profiles, currentUser }: Props) {
  ...
  const fetchForms = useCallback(async () => {
    ...
    setTemplates(((templateRows ?? []) as ProjectFormTemplate[]).filter(isVisibleProjectFormTemplate));
  ...
  const forms = useMemo(() => {
    return instances
      .filter((instance) => instance.template && isVisibleProjectFormTemplate(instance.template as ProjectFormTemplate))
